 ...
 ;
  ...
  <div className="w-full flex flex-col md:flex-row items-center justify-between mb-10 gap-6">
    ...
    >
      ...
      <Button type="submit" variant="secondary" className="ml-3 px-4 py-2 text-white bg-blue-600 rounded-md hover:bg-blue-700 shadow-md">
        Search
      </Button>
    </form>
  </div>
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
    ...
    {searchTerm
      ? ""
      : (
        <Card key={user?._id} className="border border-gray-200 rounded-lg hover:shadow-lg transition-shadow duration-300 ease-in-out">
          <div className="flex items-center gap-4 p-6">
            ...
            <div>
              ...
              <div className="text-sm text-gray-400">
                Member since {formatDistanceToNow(new Date(user?.createdAt || "11-11-2022"))}
              </div>
            </div>
          </div>
          <Separator />
    ...
    {data?.data?.map((pay_user) => (
      <Card key={pay_user._id} className="border border-gray-200 rounded-lg hover:shadow-lg transition-shadow duration-300 ease-in-out">
        <div className="flex items-center gap-4 p-6">
          <Avatar className="h-16 w-16 border-2 border-blue-500">
            <AvatarImage src={pay_user.image} alt={`${pay_user.firstName} ${pay_user.lastName}`} />
          ...
          <div>
            ...
            <div className="text-sm text-gray-400">
              Member since {formatDistanceToNow(new Date(pay_user.createdAt))}
            </div>
          </div>
        </div>
        <Separator />
        <CardContent className="p-4">
          <ChangeUserRole id={pay_user._id} role={pay_user.auth?.role || ""} />
        </CardContent>
      </Card>
    ))}
  </div>
